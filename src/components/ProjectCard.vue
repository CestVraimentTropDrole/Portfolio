<template>
  ...
  <div v-show="show" v-on:click="showDiv" class="fixed w-full h-full flex justify-center items-center p-2 lg:p-0 inset-0 bg-gray/50 z-50">
    <div class="relative w-full lg:w-3/4 lg:grid lg:grid-cols-3 lg:aspect-video bg-white rounded-lg text-black" @click.stop>
      <button v-on:click="showDiv" class="absolute right-2 top-2 lg:right-6 lg:top-6 aspect-square p-2 rounded-full bg-white hover:bg-lightgray cursor-pointer drop-shadow-box transition duration-150">
        <img src="/icons/close.svg" alt="X"/>
      </button>

      <div class="lg:col-span-2 w-full flex flex-col items-center gap-4 lg:gap-8 p-4 lg:p-8">
        ...
        <div class="w-full flex flex-col lg:grid lg:grid-cols-2 gap-6 py-4 border-t-2 border-black">
      ...
      <img :src="`/images/${project.images[0]}`" :alt="project.name" class="w-full h-full hidden lg:block col-span-1 rounded-r-lg object-cover" />
    </div>
  </div>
</template>
